...
template <>
void hdf5_load_nd_dataset<_Float16>(hid_t file_id, const char* dataset_name_,
        int min_dim, int max_dim, Blob<_Float16>* blob, bool reshape) {
  UGLY_TO_BE_IMPLEMENT;
  // hdf5_load_nd_dataset_helper(file_id, dataset_name_, min_dim, max_dim, blob,
  //                             reshape);
  // herr_t status = H5LTread_dataset__short(
  //   file_id, dataset_name_, blob->mutable_cpu_data());
  // CHECK_GE(status, 0) << "Failed to read _Float16 dataset " << dataset_name_;
}
...
template <>
void hdf5_save_nd_dataset<_Float16>(
    hid_t file_id, const string& dataset_name, const Blob<_Float16>& blob,
    bool write_diff) {
